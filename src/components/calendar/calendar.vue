...
<script>
	...
	let getDaysInOneMonth = (mydate) => { //通过获取下月面0号的日期可以知道这个月有多少天
	...
	let getlist = (mydate, that) => { //渲染出来当前list
		...
		for(let i = 0; i < getDaysInOneMonth(mydate); i++) {
			if(dateformat(new Date()) == dateformat(new Date(mydate.getFullYear() + '/' + (mydate.getMonth() + 1) + '/' + (i + 1)))) {
				array = array.concat({ //如果当前这天是今天 is_today是true
					id: i + 1,
